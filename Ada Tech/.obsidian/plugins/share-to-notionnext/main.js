...
var i18nConfig = I18n[storedLanguage || window.navigator.language.split("-")[0] || "en"];
...
var Upload2NotionNext = class extends UploadBaseNext {
  ...
  createPage(title, emoji, cover, tags, type, slug, stats, category, summary, pawrod, favicon, datetime, childArr) {
    return __async(this, null, function* () {
      ...
      const response = yield fetch("https://api.notion.com/v1/pages", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + notionAPI,
 ...
 ${data.message}`);
      ...
      if (pushCount > 0) {
        for (let i2 = 0; i2 < pushCount; i2++) {
          ...
          const extraResponse = yield fetch(
 ...
 ${extraData.message}`);
  ...
  syncMarkdownToNotionGeneral(title, cover, tags, markdown, nowFile, app) {
    return __async(this, null, function* () {
      ...
      const frontmasster = (_a4 = app.metadataCache.getFileCache(nowFile)) == null ? void 0 : _a4.frontmatter;
      const { abName } = this.dbDetails;
      const notionIDKey = `NotionID-${abName}`;
      ...
      let { response, data } = res;
      if (response && response.status === 200) {
        yield updateYamlInfo(markdown, nowFile, data, app, this.plugin, this.dbDetails);
      }
      return res;
    });
  }
